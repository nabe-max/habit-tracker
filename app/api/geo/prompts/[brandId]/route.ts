...
export async function POST(
  ...
  try {
    ...
    const state = await addCustomPrompt(brandId, token, prompt);
    if (!state) {
      return NextResponse.json({ error: "アクセスできません" }, { status: 403 });
    }
    ...
    const message =
      error instanceof Error ? error.message : "プロンプトの追加に失敗しました";
    const status = message.includes("文字") || message.includes("最大") || message.includes("同じ")
      ? 400
      : 500;
